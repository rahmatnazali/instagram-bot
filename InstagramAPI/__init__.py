...
class InstagramAPI:
    ...
    def uploadPhotoFromURL(self, photo, caption = None, upload_id = None, is_sidecar = None):
        if upload_id is None:
            upload_id = str(int(time.time() * 1000))

        # getting image from url
        import urllib, os
        urllib.urlretrieve(photo, "upload_image.jpg")
        imageFile = open("upload_image.jpg", 'rb')
        print imageFile
        ...
        '_csrftoken'        : self.token,
        'image_compression' : '{"lib_name":"jt","lib_version":"1.3.0","quality":"87"}',
        'photo'             : ('pending_media_%s.jpg'%upload_id, imageFile, 'application/octet-stream', {'Content-Transfer-Encoding':'binary'})
        }
        if is_sidecar:
            data['is_sidecar']='1'
        m = MultipartEncoder(data, boundary=self.uuid)
        ...
        if response.status_code == 200:
            if self.configure(upload_id, 'upload_image.jpg', caption):
                self.expose()
        return False
